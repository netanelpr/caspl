...
_start:	
	...
	%define p_header2 ebp-STK_RES+2*PHDR_size
...
get_phdr:
	...
	add eax, ecx
	; eld_header.entry adress = eax
	mov dword [s_top + ELF_HEADER_TO_ENTRY], eax
check3:
	;for the seond header
	mov eax, dword [p_header2 + PHDR_offset]
	sub ebx, eax
	mov dword [p_header2 + PHDR_filesize], ebx
	mov dword [p_header2 + PHDR_memsize], ebx
	...
	lseek [fd], eax, SEEK_SET
	lea eax, [p_header]
	write dword [fd], eax, 2*PHDR_size
